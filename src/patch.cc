...
void GetPossibleDisparities(vector<cv::Point2f> features_left, vector<cv::Point2f> features_right, vector<cv::Point2f>* returnList){
	
	vector<cv::Point2f> dispVector;	//Will store disparities.
	vector<cv::Point2f> leftCorrs;		//Will store source of disparity points in left image.
	vector<cv::Point2f> rightCorrs;	//Will store source of disparity points in right image.
...
void MarkCorrespondences(cv::Mat img, vector<cv::Point2f> corrs){
